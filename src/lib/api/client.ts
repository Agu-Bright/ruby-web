...
export const api = {
  ...
  // Every call carries the selected branch's businessId — the backend scopes
  // hotel rooms by an explicit businessId (create via dto.businessId, reads via
  // ?businessId), NOT the parent-pinned JWT, so branch rooms file under the
  // branch (mirrors the mobile business app fix).
  businessHotelRooms: {
    list: (businessId: string) => request<import('@/lib/business-api/hotel-rooms').HotelRoom[]>('/business/hotel-rooms', { params: { businessId } }),
    detail: (roomId: string, businessId: string) => request<import('@/lib/business-api/hotel-rooms').HotelRoom>(`/business/hotel-rooms/${roomId}`, { params: { businessId } }),
    create: (data: import('@/lib/business-api/hotel-rooms').CreateHotelRoomPayload, businessId: string) => request<import('@/lib/business-api/hotel-rooms').HotelRoom>('/business/hotel-rooms', { method: 'POST', body: { businessId, ...data } }),
    update: (roomId: string, data: import('@/lib/business-api/hotel-rooms').UpdateHotelRoomPayload, businessId: string) => request<import('@/lib/business-api/hotel-rooms').HotelRoom>(`/business/hotel-rooms/${roomId}`, { method: 'PATCH', body: data, params: { businessId } }),
    archive: (roomId: string, businessId: string) => request<import('@/lib/business-api/hotel-rooms').HotelRoom>(`/business/hotel-rooms/${roomId}`, { method: 'DELETE', params: { businessId } }),
    occupancy: (startDate: string, endDate: string, businessId: string) => request<import('@/lib/business-api/hotel-rooms').RoomOccupancy[]>('/business/hotel-rooms/occupancy', { params: { startDate, endDate, businessId } }),
  },
